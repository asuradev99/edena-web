...
| `src/lib/lattice.ts` | Crystal geometry: cell parameters, fractional/Cartesian conversion, supercells, bond finding with periodic images, neighbour search, Miller planes, all 48 generated cubic operations, and exact `mapsOntoSelf`/`siteMapping`/`symmetryOrbits` |
...
| `src/examples/symmetry.ts` + `symmetry.html` | Crystal viewer: POSCAR/CONTCAR + phonopy symmetry uploads, species-coloured atoms with periodic ghost neighbours and two-tone bonds, supercell (1–3), arc-animated symmetry operations with orbit trails and a live site-mapping report, and a per-site spherical `dV` readout |
