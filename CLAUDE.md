...
| Path | Role |
|------|------|
| `src/examples/basics.ts` + `basics.html` | The basics tour, and the place to start: eight small demos that each isolate one fundamental idea — the coordinate frame (`axes3d`, `boundsBox`, integer ticks, a grid, projected `LabelLayer` labels, ortho/perspective), interpolation (one `Timeline`, three easings, absolute-time seeking, `tween`), solids with transparent sides (a glass case with a ball and a rod inside it — `box`, `sphere` and `cylinder` faces, wire outlines, and an opacity control, relying on the renderer's back-to-front sort), the geometry primitives side by side ending in a `parametricSurface`, groups with nested transforms and inherited opacity, typeset maths riding a moving point, colour from data (per-vertex colours built straight into a `Geometry`, with `viridis`/`plasma`/`ramp` as functions from [0, 1] to a colour), and a chart in 3D (`plotFrame` with `niceStep`/`tickValues`/`formatTick` and a `functionCurve`). Each demo is a self-contained factory returning its own `update`; the page gives them one device, one render loop, `?samples=1`/`?dpr=1`, and `prefers-reduced-motion` |
