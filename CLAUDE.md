...
node scripts/check-basics.mjs [port]   # the basics tour: every control, plus real drags, measured from the compositor
...
they default to port 9444 and `http://127.0.0.1:5173`). `check-basics` drives the canvas handles with
real CDP mouse events, and keeps a *neighbour* as the control for each drag: the dragged handle has to
move and the camera must not, which is the failure mode a synthetic in-page event would hide. They take the pixels from a compositor
...
| File | Role |
|------|------|
| `src/lib/math.ts` | `Vec3`/`Color`, clamp/lerp/smooth, `add`/`sub`/`dot`/`cross`/`normalize`, column-major mat4 helpers, `rgba` |
...
| `src/lib/plot.ts` | Plot helpers: `niceStep`/`tickValues`/`tickDecimals`/`formatTick`/`tickMath`, `plotFrame` (axes, major *and* minor ticks and gridlines, axis titles, anchors carrying plain text and MathML), `areaUnder`, `lineThrough`, `secantSlope`, `axes3d`, `boundsBox` |
...
| `src/lib/camera.ts` | `OrbitCamera` with pointer/wheel interaction, `project`/`projectWith`, `ray` for picking, a `locked` flag a demo sets while it drags a handle, and `projection: 'orthographic'` (default) `| 'perspective'` |
...
| `src/lib/mathtext.ts` | Tiny MathML builders (`mi`, `mn`, `mo`, `frac`, `mroot`, `msub`, `msup`, `matrix`, `cases`, `abs`, `vec`, `limit`, `summation`, `product`, `integral`, `tint`, `mathml`, …) used by labels and equations, plus `number` (real minus signs, grouped digits, exponents), `MATH_FONT_STACK`/`MATH_CSS` |
| `src/lib/derivation.ts` | `Derivation`: an animated derivation of addressable tokens and *beats* — brackets that grow around a group, terms struck through as they cancel, recolouring, highlights and notes — with a pure model (`stepCount`, `lineOffsets`, `locate`, `checkStep`) underneath |
| `src/lib/interact.ts` | Pointer picking: `rayPlane`, `rayDistance`, `screenDistance` and `attachHandles`, the drag layer that suspends orbiting while a handle is held |

### Applications

| Path | Role |
|------|------|
| `src/examples/basics.ts` + `basics.html` | The basics tour, and the place to start: twenty-five small demos that each isolate one fundamental idea — the coordinate frame (`axes3d`, `boundsBox`, integer ticks, a grid, projected `LabelLayer` labels, ortho/perspective), interpolation (one `Timeline`, three easings, absolute-time seeking, `tween`), solids with transparent sides (a glass case with a ball and a rod inside it — `box`, `sphere` and `cylinder` faces, wire outlines, and an opacity control, relying on the renderer's back-to-front sort), the geometry primitives side by side ending in a `parametricSurface`, groups with nested transforms and inherited opacity (including a 3D tumble through the Euler `orientation`, with the label anchors riding the node's own matrix), typeset maths riding a moving point, colour from data (per-vertex colours built straight into a `Geometry`, with `viridis`/`plasma`/`ramp` as functions from [0, 1] to a colour), a chart in 3D (`plotFrame` with `niceStep`/`tickValues`/`formatTick` and a `functionCurve`), and depth (translucent slabs crossed by opaque solids, relying on the opaque-first pass and the back-to-front sort), sharing (one `Geometry` scaled across hundreds of nodes, which the renderer buckets), a camera move (`yaw`/`pitch`/`distance` driven from a scrubbed position), a hand-stepped simulation (`createParticleState`/`stepParticles`), an implicit surface (`isosurface` at a level the reader moves, inside a `boundsBox` domain), streamlines (`streamlines` from `sphereSeeds`, with markers flowing along the traced polylines), and a story told in steps (four cues over one `Timeline`, fading groups and per-visual `reveal`, with a scrubber that seeks the same clock), and a vector diagram (`arrow` plus the math typesetter's `vec` accent, restating |a + b| in a readout), and a motion path (a cubic Bézier sampled into a tube, with its control polygon and an eased marker whose raw and eased parameters are both shown), surface normals (a height field sampled on a grid, with the normal from central differences drawn as a comb of arrows), layers (three groups switched with `visible`), a 3D bar chart (data scaled into one bar mesh, with `tickValues`/`niceStep` for the axis and a `smooth`-eased sort), a following camera (`camera.target` animated while the scene stands still), and the transform stack (a node's own matrix printed with the MathML `matrix` builder as its scale, yaw and tilt change), a measurement figure (a dimension line, its legs, an angle arc and three live labels), a colour map (`colorMappedSurface` with a stated, clampable range and three palettes), and an animated derivation (`Derivation` with brackets, cancellations, recolouring and beats, beside a plot that follows it). Each demo is a self-contained factory returning its own `update`; the page gives them one device, one render loop, `?samples=1`/`?dpr=1`, and `prefers-reduced-motion` |
