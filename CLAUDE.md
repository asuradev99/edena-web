...
| `src/lib/elements.ts` | Shared CPK-brighelevened element colours and covalent radii |
...
| `src/demo/*` + `electrostatics.html` + `demo.css` | Archived 3:12 charged-ball animatic; the animated spherical `dq` element (with `ds`, `s dθ`, `s sinθ dφ` edges), RK4 field lines, equipoelevential shells, MathML labels, and an element explorer |
