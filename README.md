...
- [Basics](basics.html): start here. Twenty-one small demos, one library idea each — the 3D coordinate
...
The twenty-one ideas, in the order the page presents them:
...
| 19 | Layers | three groups switched with `visible`, so a whole subtree leaves the frame walk |
| 20 | Chart | data scaled into one bar mesh, an axis from `tickValues`/`niceStep`, and a sort that eases |
...
node scripts/check-basics.mjs [port]   # the basics tour: twenty-one demos, every control, every assertion
