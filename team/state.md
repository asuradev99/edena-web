...
- **The basics tour** (`basics.html`) is where a reader starts: twenty-two demos, each isolating one
  ...
  `22 of 22 canvases drawing · 60 fps`.
...
- **Basics tour (deepcode, 2026-09-25, third session).** The user asked to start from the basics: one
  ...
  polygon; surface normals from central differences; layers switched with `visible`; a 3D bar chart whose sort eases; a camera that follows a marker; and a node's own transform matrix read back and typeset. The panels are in the order the table of contents lists them, which an audit of TOC-against-panel order now checks.
  ...
  - Verified in Chrome Beta 155 (headless Vulkan, CDP 9444, AMD rdna-2): 60 fps with all twenty-two
