...
//   1. every one of the nine demos draws something, and each draws something distinct;
...
try {
  ...
  const stageIds = ['coordinates', 'interpolation', 'transparency', 'shapes', 'groups', 'labels', 'colour', 'plot', 'depth'];
  ...
  await evaluate(`document.getElementById('transparency-spin').click(); document.getElementById('groups-spin').click(); document.getElementById('depth-spin').click();`);
  await wait(500);
  // Only these hold still on their own, so only they can prove that a control left them alone.
  const still = new Set(['coordinates', 'shapes', 'groups', 'colour', 'plot', 'depth']);
  ...
  for (const [button, stageId] of [['transparency-spin', 'transparency'], ['groups-spin', 'groups'], ['depth-spin', 'depth']]) {
  ...
  console.log('PASS: nine demos drawing distinct scenes, every control moving its own stage alone,');
