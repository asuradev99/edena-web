...
export class Compute {
    ...
    springCountBuf;
    springDataBuf;
    springPairsBuf; // unique (i,j) pairs for cylinder rendering
    springRenderCount = 0;
    posUpdatePipeline;
    velUpdatePipeline;
    bindGroup;
    ...
    upload(spheres, selectedIndex = -1) {
        ...
        for (let i = 0; i < spheres.length; i++) {
            const s = spheres[i];
            const po = i * 16;
            physData[po] = s.position[0];
            physData[po + 1] = s.position[1];
            physData[po + 2] = s.position[2];
            physData[po + 3] = s.radius;
            physData[po + 4] = s.velocity[0];
            physData[po + 5] = s.velocity[1];
            physData[po + 6] = s.velocity[2];
    ...
    _writeParams() {
        ...
        this.device.queue.writeBuffer(this.paramsBuf, 0, buf.buffer);
    }
    step(encoder) {
    ...
    scheduleReadback(encoder) {
        if (this.sphereCount === 0 || this.readbackPending || this.stagingMapping)
            return;
        encoder.copyBufferToBuffer(this.physBuf, 0, this.stagingBuf, 0, this.sphereCount * PHYS_STRIDE);
        this.readbackPending = true;
    }
    async doReadback(spheres) {
        if (!this.readbackPending || this.stagingMapping || this.sphereCount === 0)
            return;
        ...
        for (let i = 0; i < n; i++) {
            const o = i * 16;
            spheres[i].position[0] = data[o];
            spheres[i].position[1] = data[o + 1];
            spheres[i].position[2] = data[o + 2];
            spheres[i].velocity[0] = data[o + 4];
            spheres[i].velocity[1] = data[o + 5];
            spheres[i].velocity[2] = data[o + 6];
        }
