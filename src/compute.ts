import { Sphere, PhysicsParams, Bond } from "./types.js";
...
export class Compute {
  ...
  upload(spheres: Sphere[], selectedIndex = -1): void {
    ...
    for (let i = 0; i < spheres.length; i++) {
      ...
      const color: [number, number, number] = i === selectedIndex
        ? (s.type === 2 ? [0.76, 0.78, 0.82] : [0.0, 0.84, 1.0])
        : s.type === 2 ? [0.48, 0.50, 0.54] : s.color;
      rendData[ro+4] = color[0]; rendData[ro+5] = color[1]; rendData[ro+6] = color[2];
    }
  ...
  buildAndUploadSprings(spheres: Sphere[], explicitBonds?: Bond[]): Bond[] {
    ...
    if (explicitBonds) {
      for (const bond of explicitBonds) {
        ...
        for (const [i, j] of [[bond.a, bond.b], [bond.b, bond.a]]) {
          if (counts[i] >= MAX_SPRINGS_PER_ATOM) continue;
