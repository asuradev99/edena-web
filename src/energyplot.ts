...
export class EnergyPlot {
  ...
  push(ke: number, total: number): void {
    this.ke.push(ke);
    this.tot.push(total);
    if (this.ke.length > this.MAX) { this.ke.shift(); this.tot.shift(); }
    this.draw();
  }
  ...
  private draw(): void {
    ...
    const yClamp = (v: number) => Math.max(PT - 1, Math.min(PT + PH + 1, yOf(v)));
    ...
    const drawLine = (vals: number[], color: string, width = 1.5) => {
