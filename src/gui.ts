import { PhysicsParams, Integrator } from "./types.js";
...
export class GUI {
  ...
  constructor(container: HTMLElement, initialCount: number, params: PhysicsParams) {
    this.params = params;
  ...
  rebuild(particleCount: number): void {
  ...
  private build(container: HTMLElement) {
    ...
    const btnLoad = el("button", "header-btn") as HTMLButtonElement;
    ...
    this.section(body, "SIMULATION", sec => {
      this.dropdownRow<Integrator>(sec, "Integrator", [
        { value: "velocity-verlet", label: "Velocity Verlet" },
      ], this.params.integrator, v => {
        this.params.integrator = v; this.onParamsChange?.();
      });
  ...
  private section(parent: HTMLElement, title: string, fill: (sec: HTMLElement) => void): void {
