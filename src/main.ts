...
function generateSpheres(n: number, radius: number, boxHalf: number): Sphere[] {
...
function computeEnergy(spheres: Sphere[], p: PhysicsParams): { ke: number; pe: number } {
  let ke = 0, pe = 0;
  ...
  for (let i = 0; i < spheres.length; i++) {
    for (let j = i + 1; j < spheres.length; j++) {
      ...
      if (r < 1e-6 || r > p.ljCutoff * sigma) continue;
      const sr  = sigma / r;
      const sr6 = sr ** 6;
...
async function main() {
  ...
  function frame() {
    ...
    if (topbar.isPlaying()) compute.step(encoder);
    ...
    renderer.render(
      ...
      topbar.isGlowEnabled(),
      topbar.isAxesVisible(),
      compute.renderBuf,
      compute.forceLineBuf,
      compute.sphereCount,
      physParams.showForces,
      compute.springRenderCount,
    );

    device.queue.submit([encoder.finish()]);

    device.queue.onSubmittedWorkDone().then(() =>
      compute.doReadback(spheres)
        .then(() => {
